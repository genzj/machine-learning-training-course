...
def maybe_extract(filename, force=False):
    ...
    if os.path.isdir(root) and not force:
        ...
        print('Cannot find data file %s' % (filename, ))
        print('Download it from https://pan.baidu.com/s/13_-RRNkwtkxEPPPx5nB5KA password: zwq4')
        print('And save it in current directory then retry')
        exit(1)
    else:
...
def check_imgs(dataset, show_img=False):
...
for i in range(len(pred_lst)):
    ...
    print('label: %s predict: %s correct: %s' % (alpha[test_labels[i]], alpha[pred_lst[i]], correct))
